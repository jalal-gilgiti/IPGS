...
public class IPGS{
    ...
    public IPGS(IntArrayList _targetNodeSet, double _targetRatio, double _alpha, double _beta, IntArrayList _testNodeSet){
    ...
    //numSuperedges: The number of superedges in the summary. Each superedge represents a relationship or connection between supernodes.

//2: Multiplied by 2 because each superedge consists of two endpoints.

//log2(numSupernodes): The logarithm base 2 of the number of supernodes. 
    //This term represents the amount of information or bits needed to represent each supernode in the summary.

//numSubnodes: The number of subnodes in the summary. 
    //Subnodes are smaller divisions within supernodes, and this term represents the amount of information or bits needed to represent each subnode in the summary.
    ...
    public boolean checkFormat(String number) {
        ...
        public Int2IntOpenHashMap[] initializeSummary(String dataPath){
                    ...
                    if ((parts.length >= 2) && checkFormat(parts[0]) && checkFormat(parts[1])) {
                        try {
                            ...
                            if (!subAdj[srcIDX].containsKey(dstIDX)) {
                                edges[numSubedges++] = (((int)srcIDX) << 32) + dstIDX;
            ...
            //System.out.println("|Supernodes|:\t" + numSupernodes);
    ...
    for (int i=0; i<k; i++) {
        List<int[]> hashTables = new ArrayList<>(3);
        for (int j = 0; j < 3; j++) {
            hashTables.add(new int[numSupernodes]);
        }
        
        LSH lsh = new LSH(numSupernodes, numSubnodes, 50, 1,snList , insideSupernode, edges, hashTables);
        Future<int[]> result = executorService.submit(lsh);
    ...
    int [][] sorted = Arrays.stream(ans).sorted(Comparator.comparingInt(row -> Arrays.stream(row).sum()))
        .toArray(int[][]::new);
    return sorted;
}


    //Minhash Implemenatation
//    public int[][] signature(int k){
//        final int maxCore = Runtime.getRuntime().availableProcessors();
//        ExecutorService executorService = Executors.newFixedThreadPool(maxCore);
//        List<Future<int[]>> resultList = new ArrayList<>();
...
//        for (int i=0; i<k; i++)
//        {
//            F_OPH = new OnePermHashSig[numSuperedges];
